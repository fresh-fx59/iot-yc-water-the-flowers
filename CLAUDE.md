...
**WiFi Reconnection with Backoff** (v1.17.3): Fixed WiFi driver corruption from repeated `WiFi.begin()` calls during long outages. Previously: `connectWiFi()` called `WiFi.begin()` without `WiFi.disconnect()` first, blocked Core 0 for 15s per attempt (30 retries x 500ms), no backoff → ~4320 calls/day corrupted WiFi driver → system stopped functioning. Now: `NetworkManager::loopWiFi()` method (mirrors `loopMQTT()` pattern) with: (1) `WiFi.disconnect(true)` before `WiFi.begin()` to fully reset WiFi radio, (2) exponential backoff 5s→10s→20s→...→300s cap (`WIFI_RECONNECT_BACKOFF_*` in config.h), (3) short reconnect attempts (5 retries = 2.5s blocking instead of 15s), (4) `WiFi.setAutoReconnect(true)` after successful connection, (5) recovery notification via `DebugHelper::debugImportant()`. Initial boot `connectWiFi()` unchanged (30 retries). Implemented in NetworkManager.h (`loopWiFi()`, WiFi tracking fields), main.cpp (networkTask uses `loopWiFi()`), config.h (`WIFI_RECONNECT_*` constants).
